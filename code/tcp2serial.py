...
class SerialToNet(serial.threaded.Protocol):
    ...
    def data_received(self, data):
        t2saux.msg_complete = t2saux.write(data, t2saux.msg_complete)
        if self.socket is not None:
            self.socket.sendall(data)

def validatepath():
     try:
          logging.basicConfig(filename=t2saux.log_path + 'debug.log', format='%(asctime)s,%(process)d,%(levelname)s,%(message)s', level=logging.INFO, datefmt="%d-%b-%y %H:%M:%S")
     except IOError as e:
          print("Unable to open debug.log file. %s" % e)
          sys.exit(1)
     except:
          print("Unexpected error while starting logging: ", sys.exc_info())
          sys.exit(1)
          #pass
    ...
    try:
        ...
        while True:
            if args.client:
                ...
                logging.info("Opening connection to {}:{}...\n".format(host, port))
                ...
                logging.info('Waiting for connection on port {}...\n'.format(args.localport))
                client_socket, addr = srv.accept()
                logging.info('Connected by {}\n'.format(addr))
            ...
            try:
                ...
                while True:
                    try:
                        ...
                        if not data:
                            break
                        #msg_complete = t2saux.write(data, t2saux.msg_complete)
